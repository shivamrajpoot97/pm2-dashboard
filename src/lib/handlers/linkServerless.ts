...
// Save linked servers (in-memory for serverless/demo)
  ...
  try {
    ...
    return NextResponse.json(
      { 
        ...
        demoMode: true,
        note: 'This is demo mode. Links are stored in memory and will reset on deployment.'
      },
      { status: 500 }
    );
  }
...
async function createServerLink(request: NextRequest, config: {
  ...
  return NextResponse.json({
    success: true,
    message: 'Demo server link created (stored in memory)',
    data: {
      serverId,
      name: config.name,
      secret,
      ...
      installInstructions: {
        ...
        note: 'Note: This is demo mode. Links are stored in memory and will reset on redeployment.'
      }
    },
    demoMode: true
  });
...
async function listLinkedServers() {
  ...
  return NextResponse.json({
    ...
    demoMode: true,
    note: 'Demo mode: Server links are stored in memory and will reset on redeployment. Use a database for production.'
  });
}
...
async function generateNewLink(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    demoMode: true,
    note: 'Demo mode: Links are stored in memory. For production, implement persistent storage.'
  });
}
...
async function getLinkedServersStatus() {
  ...
  return NextResponse.json({
    ...
    demoMode: true,
    note: 'Demo mode: Showing sample linked servers. Data resets on deployment.'
  });
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Demo server link removed (from memory)',
      demoMode: true
    });
  } catch (error: any) {
    return NextResponse.json(
